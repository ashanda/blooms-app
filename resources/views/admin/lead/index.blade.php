              ...
              @foreach ($leads as $lead)
                <tr>
                  ...
                  <td class="align-middle text-center">
                    <span class="text-secondary text-xs font-weight-bold">{{ $lead->appointment_date_time }}</span>
                  </td>
                  <td class="align-middle">
                    @if ($lead->visibility == 'closed' && $lead->status == 'converted' && $lead->agent_id == Auth::user()->id)
                    <p class="text-xs font-weight-bold mb-0">{{ "Can't edit" }}</p>
                    @else
                    <a href="{{ route('appointments.edit', $lead->id) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                      Edit
                    </a>
                    @endif
